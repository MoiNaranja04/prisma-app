...
const PHONE_REGEX = /^((\+?58)?[ -]?)?(0?4[12][ -]?[0-9]{3}[ -]?[0-9]{4})$/i;
const DOCUMENT_REGEX = /^[VJEGP]-[0-9]{5,9}$/i;
...
export function validatePhone(phone: string): {
  ...
  if (!PHONE_REGEX.test(phone)) {
    return {
      valid: false,
      error: "Formato inválido. Ejemplos: 0412-1234567, +584121234567",
    };
  }
...
export function validateDocument(document: string): {
  ...
  if (!DOCUMENT_REGEX.test(document)) {
    return {
      valid: false,
      error: "Formato inválido. Use: V-12345678, E-12345678, J-12345678",
    };
  }
...
export function formatDocument(document: string): string {
  if (!document) return "";
  return document.toUpperCase().replace(/\s/g, "");
}
